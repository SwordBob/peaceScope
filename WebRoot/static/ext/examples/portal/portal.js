...
//关键================================================
	...
	var viewport = Ext.create('Ext.Viewport', {
		...
		items : [ {
			...
			items : [ {
				html : '<img src = "' + appBaseUri + '/static/img/logo.png" width="85" height="47" />',
				width : 55
			}, {
				id : 'app-header-title',
				html : appName,
...
globalObject.openTab = function(tabId, tabTitle, tab, config) {
	...
	if (!_tab) {
		...
		_tab = Ext.create('Ext.panel.Panel', {
			...
			autoScroll : true,
			border : false,
			items : typeof (tab) == 'string' ? Ext.create('p2p.app.' + tab, config) : tab
		});
		mainTab.add(_tab);
...
globalObject.openWindow = function(winTitle, win, winWidth, config) {
	Ext.create('Ext.window.Window', {
		...
		resizable : false,
		width : winWidth || 300,
		items : typeof (win) == 'string' ? Ext.create('p2p.app.' + win, config) : win
	});
}
