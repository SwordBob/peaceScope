...
Ext.define('p2p.app.p2pManage.AccountQuery', {
	...
	initComponent : function() {
		...
		var accountQuerystore = Ext.create('Ext.data.Store', {
			model : 'ModelList',
			// autoDestroy: true,
			autoLoad : true,
			remoteSort : true,
		...
		var ttoolbar = Ext.create('Ext.toolbar.Toolbar', {
			items : [ {
				...
				handler : function(btn, eventObj) {
					var searchParams = {
						epcId : Ext.getCmp('accountQuery-epcId').getValue(),
						sysUserId : Ext.getCmp('accountQuery-sysUserName').getValue(),
						name : Ext.getCmp('accountQuery-name').getValue(),
						state : Ext.getCmp('accountQuery-state').getValue()
//						repaymentStateId : Ext.getCmp('accountQuery-repaymentDate').getValue()
					};
					Ext.apply(accountQuerystore.proxy.extraParams, searchParams);
					accountQuerystore.reload();
				}
			}, {
				xtype : 'button',
				text : '重置',
				...
				maxWidth : 60,
				handler : function(btn, eventObj) {
					Ext.getCmp('accountQuery-epcId').setValue(null);
					Ext.getCmp('accountQuery-sysUserName').setValue(null);
					Ext.getCmp('accountQuery-name').setValue(null);
					Ext.getCmp('accountQuery-state').setValue(null);
//					Ext.getCmp('accountQuery-repaymentDate').setValue(null);
				}
			} ]
		});
		...
		Ext.apply(this, {
			...
			listeners : {
				itemdblclick : function(dataview, record, item, index, e) {
					...
					var win = new App.AccountQueryWindow({
						hidden : true
					});
